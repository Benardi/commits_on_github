...
data %>%
  ...
  labs(x="File extension",y="Number of files edited by users")
```
...
data %>%
  ...
  labs(y="Point in Time",
       x="Number of files edited by users")
```
...
data %>%
  ...
  geom_bar(stat="identity") + 
  theme(axis.text.x = element_text(angle=60, hjust=1)) +
  labs(x="File extension",y="Median number of files edited by users")
```
...
* In terms of **sample** we see more clearly that those who use Java work way more during the week.
